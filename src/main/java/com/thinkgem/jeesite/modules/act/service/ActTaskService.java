...
public class ActTaskService extends BaseService {
	...
	public List<Act> todoList(Act act){
		...
		TaskQuery todoTaskQuery = taskService
				...
				.orderByTaskCreateTime()	// 根据任务创建的时间顺序查询
				.desc();					// 降序查询
		
		// 设置查询条件
		if (StringUtils.isNotBlank(act.getProcDefKey())){
			todoTaskQuery.processDefinitionKey(act.getProcDefKey());	// 流程定义的key
		}
		if (act.getBeginDate() != null){
			todoTaskQuery.taskCreatedAfter(act.getBeginDate());		// 在这个日期之后创建的任务
		}
		if (act.getEndDate() != null){
			todoTaskQuery.taskCreatedBefore(act.getEndDate());		// 在这个日期之前创建的任务
		}
		
		// 查询列表
		List<Task> todoList = todoTaskQuery.list(); // 返回一个List
...
//					List<HistoricIdentityLink> il = historyService.getHistoricIdentityLinksForProcessInstance(procInsId);
	...
	public String startProcess(String procDefKey, String businessTable, String businessId, String title, Map<String, Object> vars) {
		...
		// 更新业务表流程实例ID（根据表名称和id更新流程实例ID）
		Act act = new Act();
		act.setBusinessTable(businessTable);// 业务表名 （oa_leave、oa_travel、oa_test_audit......）
		act.setBusinessId(businessId);		// 业务表ID
		act.setProcInsId(procIns.getId());	// 流程实例ID
