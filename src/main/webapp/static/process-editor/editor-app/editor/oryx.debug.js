...
var DataManager = {
      ...
      DataManager.graft(XMLNS.XHTML,
          $$('.' + PROCESSDATA_REF)[0], [
            ...
            'div', {
              ...
              'class': (shape instanceof ORYX.Core.Canvas) ? "-oryx-canvas"
                  : undefined
            }
          ]);
  ...
  serialize: function (node, preserveNamespace) {
    ...
    if (node.nodeType == node.ELEMENT_NODE) {
      ...
      if (!preserveNamespace) {
        result += ' xmlns="' + (node.namespaceURI ? node.namespaceURI
            : XMLNS.XHTML) + '" xmlns:oryx="http://oryx-editor.org"';
      }
  ...
  __removeTripleRecursively: function (triple, continueFrom) {
    ...
    if (classes.include(
        triple.predicate.prefix + '-' + triple.predicate.name)) {
...
//		}
    ...
    this.save = function (synchronize) {
      ...
      ResourceManager.__request(
          ...
          function () {
            ResourceManager.__resourceActionSucceeded(
                this, synchronize ? RESOURCE_SAVED | RESOURCE_SYNCHRONIZED
                    : RESOURCE_SAVED, _id);
          },
          function () {
            ResourceManager.__resourceActionFailed(
...
ORYX.Core.SVG.MinMaxPathHandler = Clazz.extend({
  ...
  curvetoQuadraticRel: function (x1, y1, x, y) {
    this.calculateMinMax(
        [this._lastAbsX + x1, this._lastAbsY + y1, this._lastAbsX + x,
          this._lastAbsY + y]);
  },
  ...
  curvetoCubicSmoothRel: function (x2, y2, x, y) {
    this.calculateMinMax(
        [this._lastAbsX + x2, this._lastAbsY + y2, this._lastAbsX + x,
          this._lastAbsY + y]);
  },
...
ORYX.Core.SVG.SVGShape = Clazz.extend({
  ...
  init: function () {
    ...
    if (ORYX.Editor.checkClassType(this.element, SVGRectElement)
      ...
      if (this.element.points && this.element.points.numberOfItems) {
        for (var i = 0, size = this.element.points.numberOfItems; i < size;
            i++) {
    ...
    if (ORYX.Editor.checkClassType(this.element, SVGPathElement)) {
      ...
      var resizeMarkerMidAttr = this.element.getAttributeNS(NAMESPACE_ORYX,
          "resizeMarker-mid");
  ...
  update: function () {
    ...
    if (this.x !== this.oldX || this.y !== this.oldY || this.width
        !== this.oldWidth || this.height !== this.oldHeight) {
      switch (this.type) {
        case "Rect":
          if (this.x !== this.oldX) {
          ...
          if (this.y !== this.oldY) {
            this.element.setAttributeNS(null, "y",
                this.y);
          }
...
//					} else {
          this.radiusX = ((this.width < this.height) ? this.width : this.height)
              / 2.0;
          //}
        ...
        case "Polyline":
          ...
          if (points) {
            ...
            if (points && points.length && points.length > 1) {
              ...
              var updatedPoints = "";
              for (var i = 0; i < points.length; i++) {
                var x = (parseFloat(points[i]) - this.oldX) * widthDelta
                    + this.x;
                i++;
                var y = (parseFloat(points[i]) - this.oldY) * heightDelta
                    + this.y;
                updatedPoints += x + " " + y + " ";
              }
        ...
        case "Path":
          ...
          this.editPathHandler.init(this.x, this.y, this.oldX, this.oldY,
...
//				 	r = ((this.width < this.height) ? this.width : this.height)/2.0;
//				}
        return ORYX.Core.Math.isPointInEllipse(pointX, pointY,
            this.x + this.width / 2.0, this.y + this.height / 2.0, this.radiusX,
            this.radiusX);
        break;
      case "Ellipse":
        return ORYX.Core.Math.isPointInEllipse(pointX, pointY,
            this.x + this.radiusX, this.y + this.radiusY, this.radiusX,
            this.radiusY);
        break;
      case "Line":
        return ORYX.Core.Math.isPointInLine(pointX, pointY, this.x, this.y,
...
ORYX.Core.SVG.Label = Clazz.extend({
  ...
  construct: function (options) {
    ...
    this.edgePosition = this.node.getAttributeNS(ORYX.CONFIG.NAMESPACE_ORYX,
        'edgePosition');
    ...
    this.offsetTop = this.node.getAttributeNS(ORYX.CONFIG.NAMESPACE_ORYX,
        'offsetTop') || ORYX.CONFIG.OFFSET_EDGE_LABEL_TOP;
    if (this.offsetTop) {
      this.offsetTop = parseInt(this.offsetTop);
    }

    //get offset top
    this.offsetBottom = this.node.getAttributeNS(ORYX.CONFIG.NAMESPACE_ORYX,
        'offsetBottom') || ORYX.CONFIG.OFFSET_EDGE_LABEL_BOTTOM;
  ...
  update: function (force) {
    ...
    if (this._isChanged || x !== this.oldX || y !== this.oldY || force
        === true) {
      if (this.isVisible) {
          ...
          if (this._rotate !== undefined) {
            if (this._rotationPoint) {
              this.node.setAttributeNS(null, 'transform',
                  'rotate(' + this._rotate + ' ' + Math.floor(
                  this._rotationPoint.x) + ' ' + Math.floor(
                  this._rotationPoint.y) + ')');
            } else {
              this.node.setAttributeNS(null, 'transform',
                  'rotate(' + this._rotate + ' ' + Math.floor(x) + ' '
                  + Math.floor(y) + ')');
            }
          }
        } else {
    ...
    try {
      ...
      if (refNode) {
        ...
        for (var j = 0; j < tspans.length; j++) {
          ...
          var textLength = this._getRenderedTextLength(tspan, undefined,
              undefined, fontSize);
          ...
          if (textLength > refBoxLength) {
            ...
            for (var i = 0; i < numOfChars; i++) {
              var sslength = this._getRenderedTextLength(tspan, startIndex,
                  i - startIndex, fontSize);
              ...
              if (sslength > refBoxLength - 2) {
                ...
                if (lastSeperatorIndex <= startIndex) {
                  ...
                  newtspan.textContent = tspan.textContent.slice(startIndex,
...
//				}
  ...
  getFontSize: function (node) {
    ...
    if (!fontSize) {
      ...
      if (tspans[0] && /Firefox[\/\s](\d+\.\d+)/.test(navigator.userAgent)
  ...
  serialize: function () {
    ...
    if (this.position) {
      ...
      if (this.isAnchorLeft() && this.isAnchorLeft()
          !== this.isOriginAnchorLeft()) {
        pos.left = true;
      }
      if (this.isAnchorRight() && this.isAnchorRight()
          !== this.isOriginAnchorRight()) {
        pos.right = true;
      }
      if (this.isAnchorTop() && this.isAnchorTop()
          !== this.isOriginAnchorTop()) {
        pos.top = true;
      }
      if (this.isAnchorBottom() && this.isAnchorBottom()
          !== this.isOriginAnchorBottom()) {
        pos.bottom = true;
      }
...
ORYX.Core.Math.isPointInLine = function (pointX, pointY, lPoint1X, lPoint1Y,
  ...
  if (Math.abs(lPoint1X - lPoint2X) <= offset && Math.abs(pointX - lPoint1X)
      <= offset && pointY - Math.max(lPoint1Y, lPoint2Y) <= offset && Math.min(
          lPoint1Y, lPoint2Y) - pointY <= offset) {
    return true
  }

  // Check if the edge is horizontal
  if (Math.abs(lPoint1Y - lPoint2Y) <= offset && Math.abs(pointY - lPoint1Y)
      <= offset && pointX - Math.max(lPoint1X, lPoint2X) <= offset && Math.min(
          lPoint1X, lPoint2X) - pointX <= offset) {
    return true
  }

  if (pointX > Math.max(lPoint1X, lPoint2X) || pointX < Math.min(lPoint1X,
      lPoint2X)) {
    return false
  }

  if (pointY > Math.max(lPoint1Y, lPoint2Y) || pointY < Math.min(lPoint1Y,
      lPoint2Y)) {
    return false
  }
...
ORYX.Core.Math.isPointInEllipse = function (pointX, pointY, cx, cy, rx, ry) {
  ...
  if (cx === undefined || cy === undefined || rx === undefined || ry
      === undefined) {
...
ORYX.Core.Math.isPointInPolygone = function (pointX, pointY, polygone) {
  ...
  if (polygone[0] !== polygone[lastIndex - 1] || polygone[1]
      !== polygone[lastIndex]) {
 ...
 * @param {point} point
 */
ORYX.Core.Math.getDistanceBetweenTwoPoints = function (between1, between2,
    point) {
...
ORYX.Core.Math.getOrthogonalIdentityVector = function (point1, point2) {
  var vec = arguments.length == 1 ? point1 : ORYX.Core.Math.getIdentityVector(
      point1, point2);
  return {
    x: vec.y,
    y: -vec.x
...
ORYX.Core.Math.getDeterminant = function (m) {
  ...
  return (m.a * m.d * 1) + (m.c * m.f * 0) + (m.e * m.b * 0) - (m.e * m.d * 0)
      - (m.c * m.b * 1) - (m.a * m.f * 0);
}
...
ORYX.Core.Math.getTranslatedBoundingBox = function (node) {
  ...
  var ll = ORYX.Core.Math.getTranslatedPoint({x: bb.x, y: bb.y + bb.height},
      matrix);
  var ur = ORYX.Core.Math.getTranslatedPoint({x: bb.x + bb.width, y: bb.y},
      matrix);
  var lr = ORYX.Core.Math.getTranslatedPoint(
      {x: bb.x + bb.width, y: bb.y + bb.height}, matrix);
...
ORYX.Core.StencilSet.Stencil = {
  ...
  construct: function (jsonStencil, namespace, source, stencilSet,
    ...
    if (!this._jsonStencil.type || !(this._jsonStencil.type === "edge"
        || this._jsonStencil.type === "node")) {
  ...
  postProcessProperties: function () {
    ...
    if (this._jsonStencil.propertyPackages && this._jsonStencil.propertyPackages
        instanceof Array) {
      ...
      this._jsonStencil.propertyPackages.each((function (ppId) {
        ...
        if (pp) {
          pp.each((function (prop) {
            var oProp = new ORYX.Core.StencilSet.Property(prop, this._namespace,
                this);
    ...
    // init properties
    if (this._jsonStencil.properties && this._jsonStencil.properties
        instanceof Array) {
...
ORYX.Core.StencilSet.Property = Clazz.extend({
  ...
  construct: function (jsonProp, namespace, stencil) {
    ...
    this._jsonProp = jsonProp || ORYX.Log.error(
        "Parameter jsonProp is not defined.");
    this._namespace = namespace || ORYX.Log.error(
        "Parameter namespace is not defined.");
    this._stencil = stencil || ORYX.Log.error(
    ...
    jsonProp.id = jsonProp.id || ORYX.Log.error(
    ...
    else if (jsonProp.type === ORYX.CONFIG.TYPE_COMPLEX || jsonProp.type
        == ORYX.CONFIG.TYPE_MULTIPLECOMPLEX) {
...
ORYX.Core.StencilSet.PropertyItem = Clazz.extend({
  ...
  icon: function () {
    return (this._jsonItem.icon) ? this.property().stencil()._source + "icons/"
        + this._jsonItem.icon : "";
  },
    ...
    if (jsonItem.type === ORYX.CONFIG.TYPE_CHOICE) {
      if (jsonItem.items && jsonItem.items instanceof Array) {
        jsonItem.items.each((function (item) {
          this._items[item.value] = new ORYX.Core.StencilSet.PropertyItem(item,
              namespace, this);
        }).bind(this));
      } else {
        throw "ORYX.Core.StencilSet.Property(construct): No property items defined."
    ...
    if (existingSS) {
      ...
      if (jsonRules.layoutRules) {
        ...
        jsonRules.layoutRules.each(function (rules) {
          ...
          if (rules["ins"]) {
            layoutRules[layoutKey]["ins"] = (rules["ins"] || []).map(
                function (e) {
          ...
          if (rules["outs"]) {
            layoutRules[layoutKey]["outs"] = (rules["outs"] || []).map(
                function (e) {
                  return getDirections(e)
                })
          }
...
//		}
  ...
  morphStencils: function (args) {
    ...
    _morphStencils = _morphStencils.uniq().sort(function (a, b) {
      return baseMorphs.include(a) && !baseMorphs.include(b) ? -1
    ...
    var parseValues = function (o, v) {
      ...
      if (o && o[v + "s"] instanceof Array) {
        ["t", "r", "b", "l"].each(function (d) {
          ...
          layout[v][d] = Math.max(edgeRule ? edgeRule[d] : defaultRule[d],
              layout[v][d] || 0);
        }.bind(this));
      }
    }.bind(this)
  ...
  _hasRole: function (shape, role) {
    ...
    return isRole || shape.getStencil().id() == (shape.getStencil().namespace()
        + role);
  },
  ...
  _edgesWithRole: function (role) {
    return this._stencils.findAll(function (stencil) {
      return (stencil.roles().member(role) && stencil.type() === "edge") ? true
          : false;
    });
  },
  ...
  _nodesWithRole: function (role) {
    return this._stencils.findAll(function (stencil) {
      return (stencil.roles().member(role) && stencil.type() === "node") ? true
          : false;
    });
  },
...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
  ...
  construct: function (source, modelMetaData, editorId) {
    ...
    if (ORYX.CONFIG.BACKEND_SWITCH) {
      ...
      new Ajax.Request(
          ACTIVITI.CONFIG.contextRoot + '/editor/stencilset?version='
          + Date.now(), {
            asynchronous: false,
            method: 'get',
        ...
        if (jsonExtension.stencils) {
          $A(jsonExtension.stencils).each(function (stencil) {
            ...
            var oStencil = new ORYX.Core.StencilSet.Stencil(stencil,
                this.namespace(), this._baseUrl, this, undefined,
                defaultPosition);
  ...
  _init: function (response) {
    ...
    $A(this._jsonObject.stencils).each((function (stencil) {
      ...
      var oStencil = new ORYX.Core.StencilSet.Stencil(stencil, this.namespace(),
          this._baseUrl, this, pps, defaultPosition);
      this._stencils[oStencil.id()] = oStencil;
      this._availableStencils[oStencil.id()] = oStencil;
...
ORYX.Core.StencilSet.loadStencilSet = function (url, modelMetaData, editorId) {
  ...
  if (ORYX.Core.StencilSet._StencilSetNSByEditorInstance[editorId]) {
    ORYX.Core.StencilSet._StencilSetNSByEditorInstance[editorId].push(
        namespace);
  } else {
  ...
  if (ORYX.Core.StencilSet._rulesByEditorInstance[editorId]) {
    ORYX.Core.StencilSet._rulesByEditorInstance[editorId].initializeRules(
        stencilSet);
  } else {
...
ORYX.Core.UIObject = {
  ...
  addEventHandlers: function (node) {
    ...
    node.addEventListener(ORYX.CONFIG.EVENT_MOUSEDOWN,
        this._delegateEvent.bind(this), false);
    node.addEventListener(ORYX.CONFIG.EVENT_MOUSEMOVE,
        this._delegateEvent.bind(this), false);
    node.addEventListener(ORYX.CONFIG.EVENT_MOUSEUP,
        this._delegateEvent.bind(this), false);
    node.addEventListener(ORYX.CONFIG.EVENT_MOUSEOVER,
        this._delegateEvent.bind(this), false);
    node.addEventListener(ORYX.CONFIG.EVENT_MOUSEOUT,
        this._delegateEvent.bind(this), false);
    node.addEventListener('click', this._delegateEvent.bind(this), false);
    node.addEventListener(ORYX.CONFIG.EVENT_DBLCLICK,
        this._delegateEvent.bind(this), false);
        ...
        serialize.each((function (obj) {
          ...
          switch (prefix + "-" + name) {
            ...
            default:
              ...
              } else if (!(name === "bounds" || name === "parent" || name
                  === "target" || name === "dockers" || name === "docker"
      ...
      toJSON: function () {
        var json = {
          ...
          properties: jQuery.extend({}, this.properties,
            ...
            if (this.getStencil().property(key)
                && this.getStencil().property(key).type()
                === ORYX.CONFIG.TYPE_COMPLEX
              ...
              // Parse date
            } else if (value instanceof Date && this.getStencil().property(
                key)) {
              try {
                value = value.format(
                    this.getStencil().property(key).dateFormat());
              } catch (e) {
              }
            }
        ...
        if (this.dockers) {
          json.dockers = this.dockers.map(function (docker) {
            var d = docker.getDockedShape() && docker.referencePoint
                ? docker.referencePoint : docker.bounds.center();
...
ORYX.Core.Canvas = ORYX.Core.AbstractShape.extend({
  ...
  construct: function (options, stencil, facade) {
    ...
    this.columnHightlight1 = ORYX.Editor.graft("http://www.w3.org/2000/svg",
        this.underlayNode,
        ['rect', {
          x: 0,
    ...
    this.columnHightlight2 = ORYX.Editor.graft("http://www.w3.org/2000/svg",
        this.underlayNode,
        ['rect', {
...
*/
      ...
      if (!(this.children.member(uiObject))) {
        ...
        if (this.eventHandlerCallback && silent !== true) {
          this.eventHandlerCallback(
              {type: ORYX.CONFIG.EVENT_SHAPEADDED, shape: uiObject})
        }
      } else {
  ...
  addShapeObjects: function (shapeObjects, eventHandler) {
    ...
    var addShape = function (shape, parent) {
      ...
      // Create a new Shape
      var ShapeClass = (stencil.type() == "node") ? ORYX.Core.Node
          : ORYX.Core.Edge;
      ...
      shape.parent = "#" + ((shape.parent && shape.parent.resourceId)
          || parent.resourceId);
  ...
  _delegateEvent: function (event) {
    if (this.eventHandlerCallback && (event.target == this.rootNode
...
function init() {
  ...
  // Else fetch the model from server and display editor
    ...
    //修改item获取方式，原来是跳转新页面，获取页面的传参，现在放入sessionStorage中
    var item = sessionStorage.getItem("modelId");
    sessionStorage.removeItem("modelId");
    var modelId =item?item:window.location.search.substring(4);
...
ORYX.Editor = {
  ...
  construct: function (config) {
    ...
    if (ORYX.CONFIG.BACKEND_SWITCH) {
      var ssUrl = (model.stencilset.namespace || model.stencilset.url).replace(
          "#", "%23");
        ...
        if (!match.notUsesIn || !match.notUsesIn.namespaces
          ...
          try {
            ...
            this.loadScript("plugins/scripts/" + match.source,
                this.activatePluginByName.bind(this, match.name, callback,
                    true));
          }
        } else {
  ...
  loadPlugins: function () {
    ...
    ORYX.availablePlugins.each(function (value) {
      ...
      if ((!value.requires || !value.requires.namespaces
          || value.requires.namespaces.any(function (req) {
            return loadedStencilSetsNamespaces.indexOf(req) >= 0
          })) &&
          (!value.notUsesIn || !value.notUsesIn.namespaces
  ...
  getJSON: function () {
    ...
    canvasJSON.ssextensions = this.getStencilSets().values()[0].extensions().keys().findAll(
        function (sse) {
  ...
  importJSON: function (jsonObject, noSelectionAfterImport) {
    ...
    if (jsonObject.stencilset.namespace && jsonObject.stencilset.namespace
                ...
                if (dockers) {
                  if (dockers.length > 0) {
                    this.connections.push(
                        [dockers.first(), dockers.first().getDockedShape(),
                          dockers.first().referencePoint]);
                  }
                  if (dockers.length > 1) {
                    this.connections.push(
                        [dockers.last(), dockers.last().getDockedShape(),
  ...
  renewResourceIds: function (jsonObject) {
    ...
    resourceIds.each(function (oldResourceId) {
      ...
      serJsonObject = serJsonObject.replace(new RegExp(oldResourceId, 'g'),
          newResourceId);
    });
  ...
  getExtensionForMetaData: function () {
    if (!this.ss_extensions_def || !(this.ss_extensions_def.extensions
        instanceof Array)) {
      return null;
    }
  ...
  unregisterOnEvent: function (eventType, callback) {
    if (this.DOMEventListeners.keys().member(eventType)) {
      this.DOMEventListeners[eventType] = this.DOMEventListeners[eventType].without(
          callback);
    } else {
          ...
          if (pluginData.toggle === true && pluginData.buttonInstance) {
            this.registerOnEvent(eventName, function () {
              pluginData.buttonInstance.toggle(
                  !pluginData.buttonInstance.pressed); // Toggle
              pluginData.functionality.call(pluginData,
                  pluginData.buttonInstance, pluginData.buttonInstance.pressed); // Call function
            });
          } else {
  ...
  setSelection: function (elements, subSelectionElement, force) {
    ...
    if (!force && this.isEqual(this.selection, elements) && !this.isDirty(
        elements)) {
      return;
    }
  ...
  createShape: function (option) {
    ...
    if (option.connectingType && option.connectedShape && !(newShapeObject
      ...
      var cPoint = magnet ? magnet.bounds.center()
      ...
      con.dockers.last().setReferencePoint(
          newShapeObject.getDefaultMagnet().bounds.center());
    ...
    if (newShapeObject instanceof ORYX.Core.Edge && option.connectedShape) {
      ...
      if (option.connectedShape instanceof ORYX.Core.Node) {
        newShapeObject.dockers.first().setReferencePoint(
      ...
      b.moveBy(-Math.max(lwR.x - canvas.bounds.width(), 0),
...
*/
    ...
    } else if (!currentIsSelectable && currentIsMovable && !(elementController
        instanceof ORYX.Core.Controls.Docker)) {
...
ORYX.Editor.setMissingClasses = function () {
  ...
  try {
    ...
    SVGRectElement = document.createElementNS('http://www.w3.org/2000/svg',
        'rect').toString();
    SVGImageElement = document.createElementNS('http://www.w3.org/2000/svg',
        'image').toString();
    SVGCircleElement = document.createElementNS('http://www.w3.org/2000/svg',
        'circle').toString();
    SVGEllipseElement = document.createElementNS('http://www.w3.org/2000/svg',
        'ellipse').toString();
    SVGLineElement = document.createElementNS('http://www.w3.org/2000/svg',
        'line').toString();
    SVGPolylineElement = document.createElementNS('http://www.w3.org/2000/svg',
        'polyline').toString();
    SVGPolygonElement = document.createElementNS('http://www.w3.org/2000/svg',
        'polygon').toString();
...
new function () {
  ...
  ORYX.Core.UIDisableDrag = function (event) {
    document.documentElement.removeEventListener(ORYX.CONFIG.EVENT_MOUSEMOVE,
        this.dragCallback, false);
  ...
  ORYX.Core.MoveDockersCommand = ORYX.Core.Command.extend({
    ...
    execute: function () {
      ...
      this.edges.each(function (edge) {
        ...
        if (this.changes[edge.value.getId()]) {
          this.changes[edge.value.getId()].dockerPositions = edge.value.dockers.map(
              function (r) {
                return r.bounds.center()
              })
        }
    ...
    updateEdge: function (edge) {
      edge._update(true);
      [edge.getOutgoingShapes(), edge.getIncomingShapes()].flatten().invoke(
          "_update", [true])
    },
...
ORYX.Core.Shape = {
            ...
            if (property.type() == ORYX.CONFIG.TYPE_CHOICE) {
              ...
              property.items().each((function (item) {
                item.refToView().each((function (itemRef) {
                  ...
                  if (!refreshedSvgElements[svgElem.id] || prop
                      == item.value()) {
                  ...
                  if (ORYX.Editor.checkClassType(svgElem, SVGImageElement)) {
                    svgElem.setAttributeNS('http://www.w3.org/1999/xlink',
                        'href',
                        svgElem.getAttributeNS('http://www.w3.org/1999/xlink',
                            'href'));
                  }
                }).bind(this));
              }).bind(this));
              ...
              property.refToView().each((function (ref) {
                ...
                if (property.type() === ORYX.CONFIG.TYPE_KISBPM_MULTIINSTANCE) {
                  if (ref === "multiinstance") {
                    ...
                    if (svgElemParallel) {
                      if (prop === 'Parallel') {
                        svgElemParallel.setAttributeNS(null, 'display',
                            'inherit');
                      }
                      else {
                    ...
                    if (svgElemSequential) {
                      if (prop === 'Sequential') {
                        svgElemSequential.setAttributeNS(null, 'display',
                            'inherit');
                      }
                      else {
                        svgElemSequential.setAttributeNS(null, 'display',
                ...
                if (property.complexAttributeToView()) {
                  ...
                  switch (property.type()) {
                    case ORYX.CONFIG.TYPE_BOOLEAN:
                      ...
                      svgElem.setAttributeNS(null, 'display',
                          (!(prop === property.inverseBoolean())) ? 'inherit'
                              : 'none');
                      ...
                      if (property.fill()) {
                        if (svgElem.tagName.toLowerCase() === "stop") {
                          if (prop) {
                            ...
                            if (property.lightness() && property.lightness()
                                !== 1) {
                              prop = ORYX.Utils.adjustLightness(prop,
                                  property.lightness());
                            }
                            ...
                            if (svgElem.parentNode.tagName.toLowerCase()
                                === "radialgradient") {
                              ORYX.Utils.adjustGradient(svgElem.parentNode,
                                  svgElem);
                            }
                          }
                          ...
                          if (svgElem.parentNode.tagName.toLowerCase()
                              === "radialgradient") {
                            $A(svgElem.parentNode.getElementsByTagName(
                                'stop')).each(function (stop) {
                              stop.setAttributeNS(null, "stop-opacity",
                                  prop ? stop.getAttributeNS(
                                      ORYX.CONFIG.NAMESPACE_ORYX,
                                      'default-stop-opacity') || 1 : 0);
                            }.bind(this))
                          }
                        } else {
  ...
  setOpacity: function (value, animate) {
    ...
    value = Math.max(Math.min((typeof value == "number" ? value : 1.0), 1.0),
        0.0);
        ...
        if (uiObject instanceof ORYX.Core.Node) {
          ...
          if (uiObject instanceof ORYX.Core.Controls.Docker) {
            ...
            if (this.dockers.length >= 2) {
              this.dockers.splice(
                  index !== undefined ? Math.min(index, this.dockers.length - 1)
  ...
  getIntersectionPoint: function () {
    ...
    while (true) {
      ...
      var midPointX = Math.min(includePointX, excludePointX) + ((Math.max(
          includePointX, excludePointX) - Math.min(includePointX,
          excludePointX)) / 2.0);
      var midPointY = Math.min(includePointY, excludePointY) + ((Math.max(
          includePointY, excludePointY) - Math.min(includePointY,
          excludePointY)) / 2.0);
      ...
      var length = Math.sqrt(
          Math.pow(includePointX - excludePointX, 2) + Math.pow(
          includePointY - excludePointY, 2))
  ...
  deserialize: function (serialize, json) {
    ...
    if (bounds) {
      ...
      if (this instanceof ORYX.Core.Edge) {
        ...
        this.bounds.set(parseFloat(b[0]), parseFloat(b[1]), parseFloat(b[2]),
            parseFloat(b[3]));
      }
    }
...
ORYX.Core.Controls.Docker = ORYX.Core.Controls.Control.extend({
  ...
  update: function () {
    ...
    if (this._dockedShape) {
      if (this._dockedShapeBounds && this._dockedShape
          instanceof ORYX.Core.Node) {
      ...
      if (!absoluteReferenzPoint2) {
        ...
        absoluteReferenzPoint2 = {
          x: absoluteReferenzPoint1.x + (center.x - absoluteReferenzPoint1.x)
              * -minDimension,
          y: absoluteReferenzPoint1.y + (center.y - absoluteReferenzPoint1.y)
              * -minDimension
        }
      }
      ...
      newPoint = this._dockedShape.getIntersectionPoint(absoluteReferenzPoint1,
          absoluteReferenzPoint2);
  ...
  refresh: function () {
    ...
    this._dockerNode.setAttributeNS(null, 'transform',
        'translate(' + p.x + ', ' + p.y + ')');
  ...
  preventHiding: function (prevent) {
    this._preventHiding = Math.max(0,
...
ORYX.Core.Node = {
    ...
    if (this.isChanged) {
      ...
      if (this.isResized) {
        ...
        this._svgShapes.each(function (svgShape) {
          ...
          if (rightIncluded) {
            anchorOffset = oldBounds.width() - (svgShape.oldX
                + svgShape.oldWidth);
            if (leftIncluded) {
            ...
            if (!svgShape.isHorizontallyResizable) {
              svgShape.x = svgShape.x + svgShape.width * widthDelta / 2
                  - svgShape.width / 2;
            }
          }
          ...
          if (bottomIncluded) {
            anchorOffset = oldBounds.height() - (svgShape.oldY
                + svgShape.oldHeight);
            if (topIncluded) {
            ...
            if (!svgShape.isVerticallyResizable) {
              svgShape.y = svgShape.y + svgShape.height * heightDelta / 2
                  - svgShape.height / 2;
            }
          }
        });
        ...
        if (!this.isHorizontallyResizable && bounds.width()
            !== oldBounds.width()) {
          p.x = oldBounds.width() - bounds.width();
        }
        if (!this.isVerticallyResizable && bounds.height()
            !== oldBounds.height()) {
        ...
        var leftIncluded, rightIncluded, topIncluded, bottomIncluded, center,
            newX, newY;
  ...
  refresh: function () {
    ...
    // Move owner element
    this.node.firstChild.setAttributeNS(null, "transform",
        "translate(" + x + ", " + y + ")");
    // Move magnets
  ...
  isPointIncluded: function (pointX, pointY, absoluteBounds) {
    ...
    var absBounds = absoluteBounds && absoluteBounds instanceof ORYX.Core.Bounds
    ...
    outgoing.each((function (obj) {
      ...
      if (next) {
        if (next instanceof ORYX.Core.Edge) {
          ...
          next.dockers.first().setReferencePoint(
              next.dockers.first().bounds.center());
  ...
  _init: function (svgDocument) {
    ...
    var me = this;
    this._svgShapes.each(function (svgShape) {
      upperLeft.x = (upperLeft.x !== undefined) ? Math.min(upperLeft.x,
          svgShape.x) : svgShape.x;
      upperLeft.y = (upperLeft.y !== undefined) ? Math.min(upperLeft.y,
          svgShape.y) : svgShape.y;
      lowerRight.x = (lowerRight.x !== undefined) ? Math.max(lowerRight.x,
          svgShape.x + svgShape.width) : svgShape.x + svgShape.width;
      lowerRight.y = (lowerRight.y !== undefined) ? Math.max(lowerRight.y,
          svgShape.y + svgShape.height) : svgShape.y + svgShape.height;
    ...
    var magnets = svgDocument.getElementsByTagNameNS(ORYX.CONFIG.NAMESPACE_ORYX,
        "magnets");

    if (magnets && magnets.length > 0) {

      magnets = $A(magnets[0].getElementsByTagNameNS(ORYX.CONFIG.NAMESPACE_ORYX,
          "magnet"));
      ...
      magnets.each(function (magnetElem) {
        ...
        //get anchors
        var anchors = magnetElem.getAttributeNS(ORYX.CONFIG.NAMESPACE_ORYX,
            "anchors");
        if (anchors) {
      ...
      var magnet = new ORYX.Core.Controls.Magnet();
      magnet.bounds.centerMoveTo(this.bounds.width() / 2,
    ...
    /**initialize labels*/
    var textElems = svgNode.getElementsByTagNameNS(ORYX.CONFIG.NAMESPACE_SVG,
        'text');
  ...
  fitToLabels: function () {
    ...
    if (this.minimumSize) {
      ...
      if (y < minHeight && bounds.height() > minHeight && minHeight
          > this.forcedHeight) {
        bounds.set(bounds.upperLeft().x, bounds.upperLeft().y,
            bounds.lowerRight().x, bounds.upperLeft().y + minHeight);
        boundsChanged = true;
      } else if (y > minHeight && bounds.height() != y && y
          > this.forcedHeight) {
        bounds.set(bounds.upperLeft().x, bounds.upperLeft().y,
            bounds.lowerRight().x, bounds.upperLeft().y + y);
        boundsChanged = true;
      } else if (bounds.height() > this.forcedHeight && this.forcedHeight > 0) {
        bounds.set(bounds.upperLeft().x, bounds.upperLeft().y,
            bounds.lowerRight().x, bounds.upperLeft().y + this.forcedHeight);
        boundsChanged = true;
      }
    }
...
ORYX.Core.Edge = {
  ...
  _update: function (force) {
    if (this._dockerUpdated || this.isChanged || force) {
      ...
      var oldWidth = this._oldBounds.width() === 0 ? this.bounds.width()
          : this._oldBounds.width();
      var oldHeight = this._oldBounds.height() === 0 ? this.bounds.height()
      ...
      this.getLabels().each(function (label) {
        ...
        if (label.getReferencePoint()) {
          ...
          if (fromPosition.x === ref.segment.fromPosition.x && fromPosition.y
              === ref.segment.fromPosition.y &&
              toPosition.x === ref.segment.toPosition.x && toPosition.y
              === ref.segment.toPosition.y && !ref.dirty) {
            return;
          }

          if (!this.parent.initializingShapes) {
            var oldDistance = ORYX.Core.Math.getDistanceBetweenTwoPoints(
                ref.segment.fromPosition, ref.segment.toPosition,
                ref.intersection);
            ...
            var oiv = ORYX.Core.Math.getOrthogonalIdentityVector(fromPosition,
                toPosition);
            ...
            var mx = isHor && ref.orientation && (ref.iorientation
                || ref.orientation).endsWith("r") ? -label.getWidth() : 0;
            var my = isVer && ref.orientation && (ref.iorientation
                || ref.orientation).startsWith("l") ? -label.getHeight() + 2
                : 0;
            label.setX(oiv.x + mx);
            label.setY(oiv.y + my);
        ...
        if (label.position && !this.parent.initializingShapes) {
          ...
          if (y > this.bounds.lowerRight().y) {
            y += this.bounds.height() - (this.bounds.height() / (diffHeight
                || 1));
          }
          label.setX(x);
          label.setY(y);
        ...
        switch (label.getEdgePosition()) {
          ...
          case "midtop":
            ...
            if (numOfDockers % 2 == 0) {
              var angle = this._getAngle(this.dockers[numOfDockers / 2 - 1],
    ...
    if (navigator.appVersion.indexOf("MSIE 10") !== -1 || (userAgent.indexOf(
        'Trident') !== -1 && userAgent.indexOf('rv:11') !== -1)) {
  ...
  findEdgeSegmentForNode: function (node) {
    ...
    for (i = 1; i < length; i++) {
      ...
      if ((distance || distance == 0) && ((!smallestDistance && smallestDistance
          != 0)
  ...
  findSegment: function (node) {
    ...
    var nodeCenterPoint = node instanceof ORYX.Core.UIObject
        ? node.bounds.center() : node;
    ...
    for (i = 1; i < length; i++) {
      ...
      if (typeof distance == "number" && (result === undefined || distance
          < result.distance)) {
        result = {
          distance: distance,
  ...
  refresh: function () {
    ...
    if (this.getChildNodes().length > 0) {
      ...
      this.node.firstChild.childNodes[1].setAttributeNS(null, "transform",
          "translate(" + x + ", " + y + ")");
    }
  ...
  getIntersectionPoint: function () {
    ...
    return ORYX.Core.Math.midPoint(this.dockers[length - 1].bounds.center(),
  ...
  isBoundsIncluded: function (bounds) {
    ...
    return dockers.any(function (docker, i) {
      ...
      return ORYX.Core.Math.isRectOverLine(a.x, a.y, b.x, b.y, bounds.a.x,
          bounds.a.y, bounds.b.x, bounds.b.y);
    });
  },
  ...
  _getAngle: function (docker1, docker2) {
    var p1 = docker1 instanceof ORYX.Core.Controls.Docker
        ? docker1.absoluteCenterXY() : docker1;
    var p2 = docker2 instanceof ORYX.Core.Controls.Docker
        ? docker2.absoluteCenterXY() : docker2;
  ...
  add: function (shape) {
    ...
    if (shape instanceof ORYX.Core.Controls.Docker && this.dockers.include(
        shape)) {
  ...
  handleChildShapesAfterAddDocker: function (docker) {
    ...
    var relativDockerPosition = lengthSegmentPart1 / (lengthSegmentPart1
        + lengthSegmentPart2);

    segmentElements.each(function (nodePositionData) {
      /* Assign child node to the new segment */
      if (nodePositionData.value.relativDistanceFromDocker1
    ...
    this.getLabels().each(function (label) {
      ...
      if (index >= ref.segment.fromIndex && index <= ref.segment.toIndex) {
        ...
        if (!segment) {
          ...
          segment.fromDocker = ref.segment.fromIndex >= (this.dockers.length
              / 2) ? this.dockers[0] : this.dockers[this.dockers.length - 2];
  ...
  handleChildShapesAfterRemoveDocker: function (docker) {
    ...
    this.getLabels().each(function (label) {
      ...
      if (!segment) {
        ...
        from = from === docker ? this.dockers[this.dockers.indexOf(to) - 1]
            : from;
        to = this.dockers[this.dockers.indexOf(from) + 1];
      }

      var intersection = ORYX.Core.Math.getPointOfIntersectionPointLine(
          from.bounds.center(), to.bounds.center(), ref.intersection, true);
      // Update the reference point
    ...
    this._dockersByPath.any((function (pair) {
      return pair.value.any((function (docker, index) {
        if (!lastDocker) {
          ...
          if (ORYX.Core.Math.isPointInLine(position.x, position.y, point1.x,
              point1.y, point2.x, point2.y, 10)) {
            ...
            var newDocker = (exDocker) ? exDocker : this.createDocker(
                this.dockers.indexOf(lastDocker) + 1, position);
  ...
  removeUnusedDockers: function () {
    ...
    this.dockers.each(function (docker, i) {
      ...
      var cp = previous.getDockedShape() && previous.referencePoint
          ? previous.getAbsoluteReferencePoint() : previous.bounds.center();
      var cn = next.getDockedShape() && next.referencePoint
          ? next.getAbsoluteReferencePoint() : next.bounds.center();
  ...
  _init: function (svgDocument) {
    ...
    $A(g.childNodes).each((function (path, index) {
      if (ORYX.Editor.checkClassType(path, SVGPathElement)) {
        ...
        if (markerUrl && markerUrl !== "") {
          ...
          path.setAttributeNS(null, "marker-start",
              "url(#" + markerStartId + ")");
    ...
    this._paths.each((function (path) {
      paths.push(
          this.node.childNodes[0].childNodes[0].childNodes[0].appendChild(
              path));
    }).bind(this));
  ...
  serialize: function () {
    ...
    this._dockersByPath.each((function (pair) {
      pair.value.each(function (docker) {
        var position = docker.getDockedShape() && docker.referencePoint
            ? docker.referencePoint : docker.bounds.center();
      ...
      if (next) {
        if (next == targetShape) {
          ...
          this.dockers.last().setReferencePoint(
              {x: next.bounds.width() / 2.0, y: next.bounds.height() / 2.0});
    ...
    var Command = ORYX.Core.Command.extend({
      ...
      execute: function () {
        ...
        .each(function (edge) {
          if (edge.dockers.length == 2) {
            var p1 = edge.dockers.first().getAbsoluteReferencePoint()
                || edge.dockers.first().bounds.center();
            var p2 = edge.dockers.last().getAbsoluteReferencePoint()
                || edge.dockers.first().bounds.center();
            // Find all horizontal/vertical edges
            if (Math.abs(-Math.abs(p1.x - p2.x) + Math.abs(this.offset.x)) < 2
                || Math.abs(-Math.abs(p1.y - p2.y) + Math.abs(this.offset.y))
                < 2) {
              this.plugin.doLayout(edge);
            }
          }
      ...
      align: function (bounds, refDocker) {
        ...
        if (this.offset.xs !== undefined || this.offset.ys !== undefined) {
          ...
          xdif = bounds.center().x - (absPXY.x + ((abRef.x - absPXY.x)
              / this.offset.xs));
          ydif = bounds.center().y - (absPXY.y + ((abRef.y - absPXY.y)
              / this.offset.ys));
...
//    },
  ...
  editCopy: function (will_update, useNoOffset) {
    ...
    this.clipboard.refresh(selection, this.getAllShapesToConsider(selection),
  ...
  editPaste: function () {
    ...
    canvas.eachChild(function (shape, parent) {
      ...
      if (shape.target && !(childShapeResourceIds.include(
          shape.target.resourceId))) {
    ...
    canvas.eachChild(function (shape, parent) {
      ...
      if (shape.dockers) {
        shape.dockers = shape.dockers.map(function (docker, i) {
          ...
          if ((shape.targetRemoved === true && i == shape.dockers.length - 1
              && docker.getDocker) ||
              ...
              && (docker.getDocker().getDockedShape()
                  || ((outgoings[shape.resourceId] || []).length > 0
                      && (!(shape.getShape() instanceof ORYX.Core.Node)
                          || outgoings[shape.resourceId][0].getShape()
                          instanceof ORYX.Core.Node)))) ||
      ...
      } else if (shape.getShape() instanceof ORYX.Core.Node && shape.dockers
          && shape.dockers.length > 0 && (!shape.dockers.first().getDocker
              || shape.sourceRemoved === true
              || !(shape.dockers.first().getDocker().getDockedShape()
                  || outgoings[shape.resourceId]))) {
...
ORYX.Plugins.Edit.DeleteCommand = ORYX.Core.Command.extend({
  ...
  rollback: function () {
    this.shapesAsJson.each(function (shapeAsJson) {
      var shape = shapeAsJson.getShape();
      var parent = this.facade.getCanvas().getChildShapeByResourceId(
          shapeAsJson.parent.resourceId) || this.facade.getCanvas();
...
ORYX.Plugins.View = {
  ...
  zoom: function (factor) {
    ...
    /* Set new top offset */
    var offsetTop = (canvas.node.parentNode.parentNode.parentNode.offsetHeight
        - newHeight) / 2.0;
    ...
    /*readjust scrollbar*/
    var newScrollTop = scrollNode.scrollTop - Math.round(
        (canvas.getHTMLContainer().parentNode.getHeight() - newHeight) / 2)
        + this.diff;
    var newScrollLeft = scrollNode.scrollLeft - Math.round(
    ...
    /* Set Scale-Factor */
    canvas.node.setAttributeNS(null, "transform",
  ...
  zoomFitToModel: function () {
    ...
    var zoomFactor = scaleFactorHeight < scaleFactorWidth ? scaleFactorHeight
        : scaleFactorWidth;
    ...
    scrollNode.scrollTop = Math.round(bounds.upperLeft().y * this.zoomLevel)
        - 5;
    scrollNode.scrollLeft = Math.round(bounds.upperLeft().x * this.zoomLevel)
        - 5;
  ...
  resize: function (position, shrink) {
    ...
    resizeCanvas = function (position, extentionSize, facade) {
      ...
      if (position == "N" || position == "W") {
        ...
        var move = position == "N" ? {x: 0, y: extentionSize}
            : {x: extentionSize, y: 0};
...
ORYX.Plugins.CanvasResizeButton = Clazz.extend({
  ...
  construct: function (canvas, position, callback) {
    ...
    var isOverOffset = function (event) {
      ...
      if (isOverButton) {
        if (event.target.id == idGrow || event.target.id == idShrink ||
            event.target.parentNode.id == idGrow || event.target.parentNode.id
            == idShrink) {
          return true;
        } else {
          return false;
      ...
      if (event.target != parentNode && event.target != scrollNode
          && event.target != scrollNode.firstChild && event.target
          != svgRootNode && event.target != scrollNode) {
        return false;
      }
    ...
    var showButtons = (function () {
      ...
      if (position == "N" && (h - ORYX.CONFIG.CANVAS_RESIZE_INTERVAL
          > ORYX.CONFIG.CANVAS_MIN_HEIGHT)) {
        buttonShrink.show();
      } else if (position == "E" && (w - ORYX.CONFIG.CANVAS_RESIZE_INTERVAL
          > ORYX.CONFIG.CANVAS_MIN_WIDTH)) {
        buttonShrink.show();
      } else if (position == "S" && (h - ORYX.CONFIG.CANVAS_RESIZE_INTERVAL
          > ORYX.CONFIG.CANVAS_MIN_HEIGHT)) {
        buttonShrink.show();
      } else if (position == "W" && (w - ORYX.CONFIG.CANVAS_RESIZE_INTERVAL
          > ORYX.CONFIG.CANVAS_MIN_WIDTH)) {
        buttonShrink.show();
      } else {
        buttonShrink.hide();
    ...
    buttonShrink.addEventListener(ORYX.CONFIG.EVENT_MOUSEOVER,
        function (event) {
          showButtons();
        }, true);
...
ORYX.Plugins.RenameShapes = Clazz.extend({
  ...
  actOnDBLClick: function (evt, shape) {
    ...
    props = props.findAll(function (item) {
      return !item.readonly() && (item.type() == ORYX.CONFIG.TYPE_STRING
          || item.type() == ORYX.CONFIG.TYPE_EXPRESSION || item.type()
          == ORYX.CONFIG.TYPE_DATASOURCE);
    });
    ...
    if (!nearestLabel) {
      ...
      if (!nearestLabel) {
        ...
        var diff = labels.collect(function (label) {
          ...
          var len = Math.sqrt(Math.pow(center.x - evtCoord.x, 2) + Math.pow(
              center.y - evtCoord.y, 2));
  ...
  getCenterPosition: function (svgNode, shape) {
    ...
    while (hasParent) {
      if (searchShape.getParentShape().getStencil().idWithoutNs()
          === 'BPMNDiagram') {
        hasParent = false;
      }
      else {
    ...
    if (additionalIEZoom === 1) {
      ...
      center.y = center.y - (jQuery("#canvasSection").offset().top
          * additionalIEZoom) + 5 + ((canvasScrollTop * additionalIEZoom)
          - canvasScrollTop);
      center.x = center.x - (canvasOffsetLeft * additionalIEZoom)
          + additionaloffset + ((canvasScrollLeft * additionalIEZoom)
              - canvasScrollLeft);
    }
...
ORYX.Plugins.ProcessLink = Clazz.extend({
  ...
  propertyChanged: function (option, node) {
    ...
    if (option.value && option.value.length > 0 && option.value
        != "undefined") {
  ...
  alignShapes: function (way) {
    ...
    var commandClass = ORYX.Core.Command.extend({
      ...
      setBounds: function (shape, maxSize) {
        ...
        var newBounds = {
          a: {
            x: shape.bounds.upperLeft().x - (this.maxWidth
                - shape.bounds.width()) / 2,
            y: shape.bounds.upperLeft().y - (this.maxHeight
                - shape.bounds.height()) / 2
          },
          b: {
            x: shape.bounds.lowerRight().x + (this.maxWidth
                - shape.bounds.width()) / 2,
            y: shape.bounds.lowerRight().y + (this.maxHeight
                - shape.bounds.height()) / 2
          }
        }
        ...
        if (this.maxWidth > maxSize.width) {
          ...
          newBounds.b.x = shape.bounds.lowerRight().x + (maxSize.width
              - shape.bounds.width()) / 2
        }
        ...
        if (this.maxHeight > maxSize.height) {
          ...
          newBounds.b.y = shape.bounds.lowerRight().y + (maxSize.height
              - shape.bounds.height()) / 2
        }
...
ORYX.Plugins.Save = Clazz.extend({
  ...
  updateTitle: function () {
    ...
    var value = window.document.title || document.getElementsByTagName(
        "title")[0].childNodes[0].nodeValue;

    if (this.changeDifference === 0 && value.startsWith(this.changeSymbol)) {
      window.document.title = value.slice(1);
    } else if (this.changeDifference !== 0 && !value.startsWith(
        this.changeSymbol)) {
  ...
  _hasChanges: function () {
    return this.changeDifference !== 0 || (this.facade.getModelMetaData()['new']
    ...
    if (this.dragEnable) {
      ...
      if (!this.dragIntialized) {
        ...
        if (this.isAttachingAllowed &&
            this.toMoveShapes.length == 1 && this.toMoveShapes[0]
            instanceof ORYX.Core.Node &&
...
//	},
    ...
    this.containmentParentNode = underlyingNodes.reverse().find(
        (function (node) {
          return (node instanceof ORYX.Core.Canvas) ||
              (((node instanceof ORYX.Core.Node) || ((node
                  instanceof ORYX.Core.Edge) && !noEdges))
  ...
  onResizeEnd: function () {
    ...
    if (!(this.currentShapes instanceof Array) || this.currentShapes.length
        <= 0) {
      return;
    }
  ...
  onSelectionChanged: function (event) {
    ...
    if (!elements || elements.length == 0) {
      ...
      var topLevelElements = this.facade.getCanvas().getShapesWithSharedParent(
          elements);
      ...
      this.toMoveShapes = this.toMoveShapes.findAll(function (shape) {
        return shape instanceof ORYX.Core.Node &&
            (shape.dockers.length === 0 || !elements.member(
...
//				}
      ...
      if (elements.length == 1 && elements[0].isResizable) {
        var aspectRatio = elements[0].getStencil().fixedAspectRatio()
            ...
            : undefined;
        this.resizerSE.setBounds(this.dragBounds, elements[0].minimumSize,
            elements[0].maximumSize, aspectRatio);
        this.resizerSE.show();
        this.resizerNW.setBounds(this.dragBounds, elements[0].minimumSize,
            elements[0].maximumSize, aspectRatio);
        this.resizerNW.show();
      } else {
      ...
      if (ORYX.CONFIG.GRID_ENABLED) {
        ...
        this.distPointTimeout = window.setTimeout(function () {
          ...
          var distShapes = this.facade.getCanvas().getChildShapes(true).findAll(
              function (value) {
                var parentShape = value.parent;
                while (parentShape) {
                  if (elements.member(parentShape)) {
  ...
  snapToGrid: function (position) {
    ...
    this.distPoints.each(function (value) {
      ...
      if (x !== undefined) {
        offsetX = offsetX === undefined ? x : (Math.abs(x) < Math.abs(offsetX)
            ? x : offsetX);
        if (offsetX === x) {
          gridX = gx;
        }
      ...
      if (y !== undefined) {
        offsetY = offsetY === undefined ? y : (Math.abs(y) < Math.abs(offsetY)
            ? y : offsetY);
        if (offsetY === y) {
          gridY = gy;
        }
...
ORYX.Plugins.SelectedRect = Clazz.extend({
  ...
  resize: function (bounds) {
    ...
    this.dashedArea.setAttributeNS(null, 'height',
...
ORYX.Plugins.Resizer = Clazz.extend({
  ...
  handleMouseMove: function (event) {
    ...
    if (this.aspectRatio) {
      // fixed aspect ratio
      newAspectRatio = (this.bounds.width() + offset.x) / (this.bounds.height()
          + offset.y);
  ...
  update: function () {
    ...
    if (this.bounds.width() < this.minSize.width) {
      this.bounds.set(upL.x, upL.y, upL.x + this.minSize.width,
          upL.y + this.bounds.height());
    }
    ;
    if (this.bounds.height() < this.minSize.height) {
      this.bounds.set(upL.x, upL.y, upL.x + this.bounds.width(),
          upL.y + this.minSize.height);
    }
    ;
    if (this.bounds.width() > this.maxSize.width) {
      this.bounds.set(upL.x, upL.y, upL.x + this.maxSize.width,
          upL.y + this.bounds.height());
    }
    ;
    ...
    if (additionalIEZoom === 1) {
      ...
      upL.y = upL.y - (jQuery("#canvasSection").offset().top * additionalIEZoom)
          + ((canvasScrollTop * additionalIEZoom) - canvasScrollTop) + a.f;
      upL.x = upL.x - (canvasOffsetLeft * additionalIEZoom) + additionaloffset
          + ((canvasScrollLeft * additionalIEZoom) - canvasScrollLeft) + a.e;
    }
  ...
  move: function (offset, doLayout) {
    ...
    for (var i = 0; i < this.moveShapes.length; i++) {
      ...
      if (value instanceof ORYX.Core.Node) {
        ...
        // Remove all edges which are included in the selection from the list
        .findAll(function (r) {
          return r instanceof ORYX.Core.Edge && r.dockers.first().isDocked()
              && r.dockers.last().isDocked() && !this.moveShapes.include(r)
        ...
        // Remove all edges which are included in the selection from the list
        .findAll(function (r) {
          return this.moveShapes.indexOf(r.dockers.first().getDockedShape()) > i
    ...
    for (var i = 0; i < this.moveShapes.length; i++) {
      ...
      if (currentShape instanceof ORYX.Core.Node &&
        ...
        currentShape.getOutgoingShapes((function (shape) {
          if (shape instanceof ORYX.Core.Node && !this.moveShapes.member(
              shape)) {
            parents[i].add(shape);
          }
        }).bind(this));
  ...
  handleMouseDown: function (event, uiObj) {
    ...
    if (uiObj instanceof ORYX.Core.Controls.Docker && uiObj.isMovable) {
      ...
      if (uiObj.parent instanceof ORYX.Core.Edge &&
          (uiObj.parent.dockers.first() == uiObj || uiObj.parent.dockers.last()
              == uiObj)) {
      ...
      if (this.isStartDocker || this.isEndDocker) {
        ...
        if (this.docker.isDocked()) {
          ...
          var distanceDockerPointer =
              ORYX.Core.Math.getDistancePointToPoint(evPos,
        ...
        if (this.lastUIObj == uiObj) {
          ...
          if (this.docker.parent instanceof ORYX.Core.Edge) {
            ...
            /* Ensure that the shape to dock is not a child shape
							 * of the same edge.
							 */
            if (highestParent instanceof ORYX.Core.Edge && this.docker.parent
                === highestParent) {
              this.isValid = false;
              this.dockerParent._update();
              return;
            ...
            while (!this.isValid && curObj && !(curObj
                instanceof ORYX.Core.Canvas)) {
              uiObj = curObj;
        ...
        if (this.lastUIObj && this.isValid && !(event.shiftKey
            || event.ctrlKey)) {
    ...
    if (!(event.shiftKey || event.ctrlKey) && !snapToMagnet) {
      ...
      if (this.docker.parent) {
        ...
        this.docker.parent.dockers.each((function (docker) {
          ...
          var center = docker.referencePoint
              ? docker.getAbsoluteReferencePoint() : docker.bounds.center();
        ...
        if (Math.abs(nearestX) < minOffset || Math.abs(nearestY) < minOffset) {
          ...
          var previous = this.docker.parent.dockers[Math.max(
              this.docker.parent.dockers.indexOf(this.docker) - 1, 0)];
          var next = this.docker.parent.dockers[Math.min(
              this.docker.parent.dockers.indexOf(this.docker) + 1,
              this.docker.parent.dockers.length - 1)];

          if (previous && next && previous !== this.docker && next
              !== this.docker) {
            ...
            if (ORYX.Core.Math.isPointInLine(cd.x, cd.y, cp.x, cp.y, cn.x, cn.y,
                10)) {
              // Get the rise
              var raise = (Number(cn.y) - Number(cp.y)) / (Number(cn.x)
                  - Number(cp.x));
              // Calculate the intersection point
              var intersecX = ((cp.y - (cp.x * raise)) - (cd.y - (cd.x
                  * (-Math.pow(raise, -1))))) / ((-Math.pow(raise, -1))
                  - raise);
  ...
  dockerMovedFinished: function (event) {
    ...
    if (this.outerDockerNotMoved) {
      ...
      var dragDockerCommand = ORYX.Core.Command.extend({
        construct: function (docker, newPos, oldPos, newDockedShape,
            oldDockedShape, facade) {
          this.docker = docker;
      ...
      if (this.docker.parent) {
        // Instanziate the dockCommand
        var command = new dragDockerCommand(this.docker,
            this.docker.getDockedShape() ? this.docker.referencePoint
                : this.docker.bounds.center(), this._commandArg.refPoint,
            this.docker.getDockedShape(), this._commandArg.dockedShape,
            this.facade);
        this.facade.executeCommands([command]);
      }
    }
...
ORYX.Plugins.AddDocker = Clazz.extend({
  ...
  newDockerCommand: function (options) {
    ...
    var commandClass = ORYX.Core.Command.extend({
      construct: function (addEnabled, deleteEnabled, edge, docker, pos,
          facade) {
...
ORYX.Plugins.SelectionFrame = Clazz.extend({
  ...
  handleMouseUp: function (event) {
    ...
    if (this.moveCallback) {
      ...
      if (additionalIEZoom === 1) {
        ...
        a.x = a.x - (corrSVG.e - (canvasOffsetLeft * additionalIEZoom)
            + additionaloffset + ((canvasScrollLeft * additionalIEZoom)
                - canvasScrollLeft));
        a.y = a.y - (corrSVG.f - (jQuery("#canvasSection").offset().top
            * additionalIEZoom) + ((canvasScrollTop * additionalIEZoom)
            - canvasScrollTop));
        b.x = b.x - (corrSVG.e - (canvasOffsetLeft * additionalIEZoom)
            + additionaloffset + ((canvasScrollLeft * additionalIEZoom)
                - canvasScrollLeft));
        b.y = b.y - (corrSVG.f - (jQuery("#canvasSection").offset().top
            * additionalIEZoom) + ((canvasScrollTop * additionalIEZoom)
      ...
      var elements = this.facade.getCanvas().getChildShapes(true).findAll(
          function (value) {
  ...
  handleMouseMove: function (event) {
    // Calculate the size
    var size = {
      width: Event.pointerX(event) - this.position.x - jQuery(
          "#canvasSection").offset().left,
      height: Event.pointerY(event) - this.position.y - jQuery(
          "#canvasSection").offset().top + 5
    };
...
ORYX.Plugins.ShapeHighlighting = Clazz.extend({
  ...
  setAttributesByStyle: function (node, options) {
    ...
    if (options.style && options.style
      ...
      var strWidth = options.strokewidth ? options.strokewidth
          : ORYX.CONFIG.BORDER_OFFSET

      node.setAttributeNS(null, "d",
          this.getPathRectangle(bo.a, bo.b, strWidth));
      node.setAttributeNS(null, "stroke", options.color ? options.color
          : ORYX.CONFIG.SELECTION_HIGHLIGHT_COLOR);
      node.setAttributeNS(null, "stroke-opacity",
  ...
  getPathCorners: function (a, b) {
    ...
    // Set: Upper left
    path = path + "M" + a.x + " " + (a.y + size) + " l0 -" + size + " l" + size
        + " 0 ";
    // Set: Lower left
    path = path + "M" + a.x + " " + (b.y - size) + " l0 " + size + " l" + size
        + " 0 ";
    // Set: Lower right
    path = path + "M" + b.x + " " + (b.y - size) + " l0 " + size + " l-" + size
        + " 0 ";
    // Set: Upper right
    path = path + "M" + b.x + " " + (a.y + size) + " l0 -" + size + " l-" + size
        + " 0 ";
...
ORYX.Plugins.HighlightingSelectedShapes = Clazz.extend({
  ...
  onSelectionChanged: function (event) {
    if (event.elements && event.elements.length > 1) {
      ...
      this.facade.raiseEvent(
          {type: ORYX.CONFIG.EVENT_HIGHLIGHT_HIDE, highlightId: 'selection'});
      this.facade.raiseEvent({
        type: ORYX.CONFIG.EVENT_HIGHLIGHT_HIDE,
        highlightId: 'subselection'
      });
...
ORYX.Plugins.Overlay = Clazz.extend({
  ...
  construct: function (facade) {
    ...
    this.facade.registerOnEvent(ORYX.CONFIG.EVENT_OVERLAY_SHOW,
        this.show.bind(this));
    this.facade.registerOnEvent(ORYX.CONFIG.EVENT_OVERLAY_HIDE,
        this.hide.bind(this));
  ...
  show: function (options) {
    ...
    if (!options ||
        ...
        !options.id || !options.id instanceof String || options.id.length
        == 0) {
    ...
    if (options.node && isSVG) {
      ...
      options.shapes.each(function (el, index) {
        ...
        var _temp = {}
        _temp.svg = options.dontCloneNode ? options.node
  ...
  setAttributes: function (node, attributes) {
    ...
    var attrMarkerValue = attributes.stroke ? attrValue.slice(0,
        attrValue.length - 1) + "; fill:" + attributes.stroke + ";}"
        : attrValue;
    var attrTextValue;
    if (attributes.fill) {
    ...
    csstags = ids.collect(function (s, i) {
      return "#" + s + " * " + (!i ? attrValue : attrMarkerValue) + ""
...
new function () {
  ...
  ORYX.Plugins.Layouter.EdgeLayouter = ORYX.Plugins.AbstractLayouter.extend({
    ...
    getPositions: function (from, to, edge) {
      ...
      //     V
      // Do nothing
      if (Math.abs(aFirst.x - aLast.x) < 1 || Math.abs(aFirst.y - aLast.y)
          < 1) {
        return []
      }
      ...
      var m = {}
      m.x = a.x < b.x ?
          (((b.x - bb.width() / 2) - (a.x + ab.width() / 2)) / 2) + (a.x
          + ab.width() / 2) :
          (((a.x - ab.width() / 2) - (b.x + bb.width() / 2)) / 2) + (b.x
          + bb.width() / 2);

      m.y = a.y < b.y ?
          (((b.y - bb.height() / 2) - (a.y + ab.height() / 2)) / 2) + (a.y
          + ab.height() / 2) :
          (((a.y - ab.height() / 2) - (b.y + bb.height() / 2)) / 2) + (b.y
          + bb.height() / 2);
    ...
    getWeight: function (from, d1, to, d2, edge, reverse) {
      ...
      if (reverse) {
        // Reverse d1 and d2
        d1 = d1 == "t" ? "b" : (d1 == "r" ? "l" : (d1 == "b" ? "t" : (d1 == "l"
            ? "r" : "r")))
        d2 = d2 == "t" ? "b" : (d2 == "r" ? "l" : (d2 == "b" ? "t" : (d2 == "l"
            ? "r" : "r")))
      }
...
new function () {
  ...
  ORYX.Plugins.BPMN2_0 = {
    ...
    construct: function (facade) {
      ...
      this.facade.registerOnEvent(ORYX.CONFIG.EVENT_PROPWINDOW_PROP_CHANGED,
    ...
    onSelectionChanged: function (event) {
      ...
      if (selection.any(function (s) {
        return s instanceof ORYX.Core.Node && s.getStencil().id().endsWith(
            "Lane")
      })) {
    ...
    handleShapeRemove: function (option) {
      ...
      if (sh instanceof ORYX.Core.Node && sh.getStencil().idWithoutNs()
          === "Lane" && this.facade.isExecutingCommands()) {
    ...
    handleSubProcess: function (option) {
      ...
      var resized = this.hashedSubProcesses[sh.id].width !== sh.bounds.width()
          || this.hashedSubProcesses[sh.id].height !== sh.bounds.height();
    ...
    moveChildDockers: function (shape, offset) {
      ...
      dockers.each(function (docker) {
        ...
        if (!abs.isIncluded(docker.bounds.center())) {
          ...
          if (!bothAreIncluded) {
            var previousIsOver = index !== 0 ? abs.isIncluded(
                docker.parent.dockers[index - 1].bounds.center()) : false;
            var nextIsOver = index !== size - 1 ? abs.isIncluded(
                docker.parent.dockers[index + 1].bounds.center()) : false;
            ...
            var ref = docker.parent.dockers[previousIsOver ? index - 1 : index
                + 1];
            if (Math.abs(
    ...
    handleDockerDocked: function (options) {
      ...
      if (edge.getStencil().id() === namespace + "SequenceFlow") {
        ...
        if (!isGateway && (edge.properties["oryx-conditiontype"]
            == "Expression"))
        ...
        if ((shape.getStencil().id() === namespace + "SequenceFlow") &&
          ...
          // Do not show the Diamond
          {
            ...
            var incomingGateway = incomingShapes.find(function (aShape) {
              var foundGateway = aShape.getStencil().groups().find(
                  function (group) {
                    if (group == "Gateways") {
                      return group;
                    }
    ...
    handleLayoutPool: function (event) {
      ...
      if (!(currentShape.getStencil().id().endsWith("Pool")
      ...
      if (lanes.length === 1 && this.getLanes(lanes.first()).length <= 0) {
        ...
        lanes.first().setProperty("oryx-showcaption",
      ...
      if (deletedLanes.length > 0 || addedLanes.length > 0) {
        ...
        if (addedLanes.length === 1 && this.getLanes(
            addedLanes[0].parent).length === 1) {
       ...
       */
        ...
        if (selection.length === 1 && this.isResized(pool,
          ...
          this.adjustLanes(pool, allLanes, oldXY.x - xy.x, oldXY.y - xy.y,
              scale);
        }
       ...
       */
        ...
        if (selection.length === 1 && this.isResized(currentShape,
            this.hashedBounds[pool.id][currentShape.id])) {
          ...
          if (x || y) {
            ...
            this.adjustLanes(pool, this.getAllExcludedLanes(pool, currentShape),
                x, 0);
          }
        ...
        width = this.adjustWidth(lanes,
            currentShape.bounds.width() + (this.getDepth(currentShape, pool)
            * 30));
      }

      this.setDimensions(pool, width, height, x, y);

      if (this.facade.isExecutingCommands() && (deletedLanes.length === 0
          || addedLanes.length !== 0)) {
        // Update all dockers
        ...
        if (this.hashedPositions[pool.id]
          ...
          var LanesHasBeenReordered = ORYX.Core.Command.extend({
            construct: function (originPosition, newPosition, lanes, plugin,
                poolId) {
          ...
          var command = new LanesHasBeenReordered(hashedPositions, hp2,
              allLanes, this, pool.id);
          this.facade.executeCommands([command]);
      ...
      lanes.each(function (l) {
        l.getChildNodes().each(function (child) {
          if (!child.getStencil().id().endsWith("Lane")) {
            var cy = scale ? child.bounds.center().y - (child.bounds.center().y
                / scale) : -y;
    ...
    getAllExcludedLanes: function (parent, lane) {
      ...
      parent.getChildNodes().each(function (shape) {
        if ((!lane || shape !== lane) && shape.getStencil().id().endsWith(
            "Lane")) {
          lanes.push(shape);
    ...
    setDimensions: function (shape, width, height, x, y) {
      ...
      shape.bounds.set(
          ...
          width ? shape.bounds.a.x + width - (isLane ? 30 : (x || 0))
              : shape.bounds.b.x,
          height ? shape.bounds.a.y + height - (isLane ? 0 : (y || 0))
              : shape.bounds.b.y
      );
    },
    ...
    getOffset: function (lane, includePool, pool) {
      ...
      var hashed = this.hashedBounds[pool.id][lane.id] || (includePool === true
          ? this.hashedPoolPositions[lane.id] : undefined);
      if (hashed) {
        ...
        while (++j < children.length) {
          ...
          while (++k < edges.length) {
            ...
            while (++l < edges[k].dockers.length) {
              ...
              var previousIsOverLane = l == 0 ? isOverLane
                  : oldBounds.isIncluded(
                      edges[k].dockers[l - 1].bounds.center());
              var nextIsOverLane = l == edges[k].dockers.length - 1 ? isOverLane
                  : oldBounds.isIncluded(
                      edges[k].dockers[l + 1].bounds.center());
              var off = Object.clone(offset);

              // If the
              if (isScaled && isOverLane && this.isResized(lanes[i],
                  this.hashedBounds[pool.id][lanes[i].id])) {
                var relY = (pos.y - absBounds.upperLeft().y + off.y);
                off.y -= (relY - (relY * (absBounds.height()
                    / oldBounds.height())));
              }
    ...
    getLanes: function (shape, recursive) {
      ...
      lanes = lanes.sort(function (a, b) {
        ...
        if (Math.round(a.bounds.height() - ha.height()) === 0 && Math.round(
            b.bounds.height() - hb.height()) === 0) {
        ...
        return (above || slightlyAboveBottom || slightlyAboveTop ? -1 : (below
        || slightlyBelowBottom || slightlyBelowTop ? 1 : 0))
      }.bind(this));
  ...
  var ResizeLanesCommand = ORYX.Core.Command.extend({
    ...
    execute: function () {
        ...
        if (laUpL.y >= shUpL.y) {
          this.lane.getChildShapes().each(function (childShape) {
            ...
            if (!this.changes[childShape.getId()]) {
              this.changes[childShape.getId()] = this.computeChanges(childShape,
                  this.lane, this.lane, this.shape.bounds.height());
            }
          ...
          this.shapeChildren.each(function (shapeChild) {
            ...
            shapeChild.shape.bounds.moveBy((shUpL.x - 30) - (depthChange * 30),
                0);
            ...
            if (!this.changes[shapeChild.shape.getId()]) {
              this.changes[shapeChild.shape.getId()] = this.computeChanges(
                  shapeChild.shape, this.shape, this.lane, 0);
            }
      ...
      var newHeight = this.lane.length === 1 ? this.parentHeight
          : this.lane.bounds.height() + this.shape.bounds.height();

      // Set height
      this.setHeight(newHeight, oldHeight, this.parent, this.parentHeight,
          true);
      ...
      this.plugin.getLanes(this.parent).each(function (childLane) {
        if (!this.changes[childLane.getId()] && childLane !== this.lane
          ...
          this.changes[childLane.getId()] = this.computeChanges(childLane,
    ...
    setHeight: function (newHeight, oldHeight, parent, parentHeight, store) {
      ...
      if (store === true) {
        // Store changes
        this.changes[this.shape.getId()] = this.computeChanges(this.shape,
            parent, parent, 0, oldHeight, newHeight);
      }
    ...
    computeChanges: function (shape, oldParent, parent, yOffset, oldHeight,
      ...
      oldParent = this.changes[shape.getId()]
          ? this.changes[shape.getId()].oldParent : oldParent;
      var oldPosition = this.changes[shape.getId()]
          ? this.changes[shape.getId()].oldPosition : shape.bounds.upperLeft();
